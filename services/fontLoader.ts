...
const getFontFilePath = (name: string, weight: number): string => {
    if (name === 'Inter') {
...
export const loadLocalFont = async (name: string, weight: number): Promise<ArrayBuffer | null> => {
...
export const loadGoogleFont = async (name: string, weight: number): Promise<ArrayBuffer | null> => {
...
export const loadFont = async (name: string, weight: number): Promise<FontConfig | null> => {
    let data = await loadLocalFont(name, weight)
    let source: 'local' | 'google' = 'local'
    let fontName = name

    if (!data) {
        data = await loadGoogleFont(name, weight)
        source = 'google'
    }

    if (!data) {
        data = await loadLocalFont('Inter', weight)
        source = 'local'
        fontName = 'Inter'
    }
...
export const getAvailableFonts = (): { local: FontInfo[]; googleFontsSupported: boolean } => {
    return {
        local: LOCAL_FONTS,
